...
### Added
- "Session already exists" modal on all three launch actions ("New session in current folder", "Create new project", "Open existing project"). When a compatible session for the target directory + agent already exists, uxon now asks whether to **attach** to the existing session or **start a new one alongside**. The modal is a plain list — each compatible session is a row, followed by a `+ Start new session alongside` row; pick a row with Enter to confirm it (a session row attaches, the trailing row opens a parallel session), `n` is a shortcut for new, `Esc` cancels. Replaces the previous silent auto-attach that ignored the operator's selected permission mode.
- Native, uxon-managed git worktrees for every agent. The launch-options screen has a new WORKSPACE column (primary tree + existing worktrees + "+ New worktree…"); pick a workspace or create one and uxon launches the agent there. The column appears for any git target in both launch flows — **New session in current folder** and **Open existing project** — and is skipped for non-git targets. New config keys `worktree_root` and `worktree_base`.
- uxon-managed tmux options, **off by default** (opt-in). When enabled, uxon applies a recommended set of tmux options — `mouse`, `allow-passthrough` (`set -g`), `extended-keys` (`set -s`), and `terminal-features=xterm*:extkeys` (`set -as`) — to the sessions it launches, layered on top of the launch user's own tmux config without editing any file. Enable with `tmux.manage_options = true` (also editable from the superuser settings screen); the recommended tables ship built-in, so the toggle alone is enough. Emission order is global → server → append-server; a rejected option fails the launch (fail-fast — no degraded session). Customise per scope via `[tmux.options]`/`[tmux.server_options]`/`[tmux.append_server_options]` in `config.toml` (omitted scopes keep their recommended defaults).

### Changed
- Per-host server metrics moved out of the way of the session list. The block that stacked one full status line per host **above** the table is replaced by a single collapsible **fleet status bar below** the table. Collapsed (default) it shows just `N hosts · M sess` plus alerts when a host is unreachable or its memory is ≥ 90 % — transient `pending…`/`(cached)` states no longer raise an alert or colour the bar. Press `h` (or click the bar) to expand it into one detail line per host. Load average (`la`) is no longer shown. In `by_host` view the active host's detail still appears under its tab.
