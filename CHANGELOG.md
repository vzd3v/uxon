...
- uxon-managed tmux options, **on by default**. uxon now applies a recommended set of tmux options — `mouse`, `allow-passthrough` (`set -g`), `extended-keys` (`set -s`), and `terminal-features=xterm*:extkeys` (`set -as`) — to every session it launches, layered on top of the launch user's own tmux config without editing any file. Emission order is global → server → append-server; a rejected option fails the launch (fail-fast — no degraded session). Override per scope via `[tmux.options]`/`[tmux.server_options]`/`[tmux.append_server_options]` in `config.toml` (omitted scopes keep their recommended defaults), or set `tmux.manage_options = false` to disable. The toggle is also editable from the superuser settings screen.
