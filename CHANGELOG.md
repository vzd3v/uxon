...
- Container-aware agent gating. With `[container]` enabled the agent is provisioned inside the container, so `uxon run` / `uxon new` and the TUI no longer require the agent binary on the host PATH and no longer block the launch (or show "agents unavailable") when it is absent. The agent is still resolved by the normal precedence; because uxon cannot auto-detect an agent from the host in this mode, an auto setup (no `--agent`, empty `agents.enabled`, no `agents.default`) must name an agent explicitly. The host probe still runs, so `uxon doctor` and version surfaces show the real host landscape. When `[container]` is disabled, host gating is byte-for-byte unchanged.
...
- `uxon doctor` container diagnostics. When `[container]` is enabled, `uxon doctor` adds a container section: the resolved name, the result of your `exists_cmd`/`is_running_cmd` probes for a representative target, a note that a host-absent agent binary is expected (not a fault), and warnings when `stop_template` is unset (the agent orphans on kill), when `path_map` doesn't cover the current directory, or when the `create_template` definition resolves to a path inside the agent-writable bind mount. Absent entirely when `[container]` is disabled.
- A uxon-managed worktree whose path falls under no `[container.path_map]` host prefix (when a non-empty `path_map` is configured) now fails fast with a clear message before `git worktree add`, instead of deferring to an opaque runtime error inside the container. An empty `path_map` (bind-at-same-path) is unaffected.
