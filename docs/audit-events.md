...
| `session.kill`       | Local `uxon kill` or TUI `k` on a local row.                                                             | `session`, `target_user`, `force` (bool), `dry_run`                                                                          | `ok`, `denied`, `error`, `not_found` |
...
| `kill.remote.out`    | Local `uxon kill --host` / TUI `k` on a remote row (caller side).                                         | `peer_name`, `ssh_alias`, `target_user`, `target_session`, `force`, `dry_run`, `correlation_id`                              | `ok`, `error`                    |
