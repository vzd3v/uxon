...
| `tmux_socket_template` | string | `/tmp/tmux-{uid}/uxon-{execution_backend}.sock` | Dedicated socket path. Its parent must be a real directory owned by the launch user with mode `0700`; the socket must be owned by that user with owner-only mode `0600` or `0700` (both are emitted by supported tmux environments). Placeholders: `{user}`, `{uid}`, `{execution_backend}`, `{execution_fingerprint}`. |
