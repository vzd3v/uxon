...
import ccw_agents  # noqa: E402


class CatalogTests(unittest.TestCase):
    def test_catalog_has_three_agents(self) -> None:
        self.assertEqual(set(ccw_agents.CATALOG), {"claude", "codex", "cursor"})
    ...
    def test_cursor_has_no_auto(self) -> None:
        cursor = ccw_agents.CATALOG["cursor"]
    ...
    def test_claude_and_codex_have_auto(self) -> None:
        for aid in ("claude", "codex"):
            ids = [m.id for m in ccw_agents.CATALOG[aid].permission_modes]
    ...
    def test_yolo_flags(self) -> None:
        self.assertEqual(
            ccw_agents.permission_mode_for(ccw_agents.CATALOG["claude"], "yolo").flags,
            ("--dangerously-skip-permissions",),
        )
        self.assertEqual(
            ccw_agents.permission_mode_for(ccw_agents.CATALOG["codex"], "yolo").flags,
            ("--dangerously-bypass-approvals-and-sandbox",),
        )
        self.assertEqual(
            ccw_agents.permission_mode_for(ccw_agents.CATALOG["cursor"], "yolo").flags,
            ("--yolo",),
        )

    def test_auto_flags(self) -> None:
        self.assertEqual(
            ccw_agents.permission_mode_for(ccw_agents.CATALOG["claude"], "auto").flags,
    ...
    def test_normal_has_no_flags(self) -> None:
        for agent in ccw_agents.CATALOG.values():
            mode = ccw_agents.permission_mode_for(agent, "normal")
            self.assertEqual(mode.flags, ())


class ProbeAgentsTests(unittest.TestCase):
    def test_probe_ok(self) -> None:
        with mock.patch("ccw_agents.subprocess.run") as run:
            run.return_value = subprocess.CompletedProcess(
                args=[], returncode=0, stdout="1.0.1\n", stderr=""
            )
            result = ccw_agents.probe_agents(["claude"], launch_user=None)
    ...
    def test_probe_missing_filenotfound(self) -> None:
        with mock.patch(
            "ccw_agents.subprocess.run",
            side_effect=FileNotFoundError("no such binary"),
        ):
            result = ccw_agents.probe_agents(["codex"], launch_user=None)
    ...
    def test_probe_missing_nonzero_exit(self) -> None:
        with mock.patch("ccw_agents.subprocess.run") as run:
            ...
            result = ccw_agents.probe_agents(["cursor"], launch_user=None)
    ...
    def test_probe_uses_sudo_when_launch_user_differs(self) -> None:
        ...
        with mock.patch("ccw_agents.subprocess.run", side_effect=fake_run):
            with mock.patch("ccw_agents._current_user", return_value="root"):
    ...
    def test_probe_unknown_agent_id_ignored(self) -> None:
        result = ccw_agents.probe_agents(["nosuch"], launch_user=None)
