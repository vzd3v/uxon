"""Event-log tests for :func:`ccw_tui.events._log_event`."""
...
from ccw_tui.events import _log_event  # noqa: E402
...
class LogEventTests(unittest.TestCase):
    ...
    def test_log_event_writes_jsonl_line(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            with mock.patch.dict(os.environ, {"CCW_LOG_DIR": td}):
                _log_event(
                    "tui_start",
                    caller_user="u",
                    launch_user="u",
    ...
    def test_log_event_swallows_errors(self) -> None:
        ...
        with mock.patch.dict(os.environ, {"CCW_LOG_DIR": "/proc/nonexistent"}):
            try:
    ...
    def test_log_event_creates_directory(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with mock.patch.dict(os.environ, {"CCW_LOG_DIR": nested}):
                _log_event("x", caller_user="u")
