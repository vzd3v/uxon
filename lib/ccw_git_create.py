...
Wires the two responsibilities together for a single ``ccw new`` call:

1. **Under launch_user**: ``git init -b main`` + empty initial commit in
   ``project_dir``. If that directory already has a ``.git``, the whole
   operation is skipped (the caller is re-running ``ccw new`` on a
...
The two backends (:mod:`ccw_git_backend_gh` and
:mod:`ccw_git_backend_token`) share the ``BackendError`` type so the
...
def _launch_prefix(launch_user: str, current_user: str) -> list[str]:
    ...
    -iu`` here (not ``-n -u``) to match the rest of ccw's launch flow.
    """
